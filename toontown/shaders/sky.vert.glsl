...
void main() {
    ...
    // The dome follows the camera, but atmosphere calculations must remain in
    // world space.  Treat the vertex as a direction (w=0) so camera translation
    // is ignored while camera rotation is correctly removed from the sky.
    vDir = (p3d_ModelMatrix * vec4(p3d_Vertex.xyz, 0.0)).xyz;
    ...
    // Anchor every celestial layer to world space.  The dome itself follows
    // the camera to stay at infinity, so model-space UVs make stars appear
    // glued to the screen when the camera turns.
    float len   = length(vDir);
    vec3  d     = vDir / max(len, 0.001);
